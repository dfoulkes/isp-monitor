...

{
  _config+:: {
    ...
    urls+:: {
      ...
      prom_ingress: ['prometheus-speed.' + domain for domain in $._config.urls.domains],
      grafana_ingress: ['grafana-speed.' + domain for domain in $._config.urls.domains],
      grafana_ingress_external: 'grafana-speed.' + vars.suffixDomain,
    },
    ...
    grafana+:: {
      config: {
        sections: {
          ...
          server: {
            root_url: vars.grafana.root_url
          },
        },
      },
  ...
  ingress+:: {
    ...
    prometheus:
      local I = utils.newIngress('prometheus-speed', $._config.namespace, $._config.urls.prom_ingress, '/', 'prometheus-k8s', 'web');
      if vars.TLSingress then
        if vars.UseProvidedCerts then
